/*
 * diffusion_problem_basis.hpp
 *
 *  Created on: Oct 7, 2019
 *      Author: heena
 */

#ifndef DIFFUSION_PROBLEM_INCLUDE_DIFFUSION_BASIS_HPP_
#define DIFFUSION_PROBLEM_INCLUDE_DIFFUSION_BASIS_HPP_
...
template <int dim>
  ...
  DiffusionProblemBasis<dim>::make_grid()
  {
    ...
    std::cout <<  " Local MultiscaleFEM Coarse --- "
  ...
  template <int dim>
  const std::string &
  DiffusionProblemBasis<dim>::get_filename_global() const
  {
  ...
  {
    this_subdomain(i) = local_subdomain;
  }
  ...
  std::ofstream output(filename_global);
  data_out.write_vtu(output);
...
#endif /* DIFFUSION_PROBLEM_INCLUDE_DIFFUSION_PROBLEM_BASIS_HPP_ */
